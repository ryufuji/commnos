...
## 2026-01-09 - 全ページへのデザインシステム適用

### 🔧 修正 (Fix)
- **全ルートファイルのCSS読み込みを修正**
  - `src/routes/tenant-public.ts` - テナント公開ページ（home, members, posts, etc.）
  - `src/routes/tenant.ts` - テナントオーナーページ
  - `src/routes/password-reset-pages.ts` - パスワードリセットページ
  - 古い `/static/styles.css` を完全削除
  - すべてのページに `commons-theme.css` と `commons-components.css` を適用

### 🎨 デザインユーティリティクラス追加
- **commons-components.css に187行追加**
  - カラーユーティリティ: `text-primary-500`, `text-accent-500`, `text-success-500`, `text-gradient`
  - ボタンスタイル: `btn-primary`, `btn-secondary`
  - カードスタイル: `card`, `card-hover`, `card-interactive`
  - バッジ: `badge-primary`, `badge-accent`
  - アニメーション: `fade-in`, `slideDown`

### 📊 影響範囲
**修正されたルートファイル**: 4件
1. `src/index.tsx` - トップページ、登録ページなど
2. `src/routes/tenant-public.ts` - テナント公開ページ（15箇所）
3. `src/routes/tenant.ts` - テナントオーナーページ
4. `src/routes/password-reset-pages.ts` - パスワードリセット

**デザインが適用されたページ**:
- ✅ `/` - トップページ
- ✅ `/communities` - コミュニティ一覧
- ✅ `/tenant/home?subdomain=test` - テナントホーム
- ✅ `/tenant/members?subdomain=test` - メンバー一覧
- ✅ `/tenant/posts?subdomain=test` - 投稿一覧
- ✅ その他すべてのページ
...
**After（修正後）**:
- ✅ ターコイズ (#00BCD4) - プライマリカラー
- ✅ イエロー (#FDB714) - アクセントカラー
- ✅ グリーン (#10B981) - 成功カラー
- ✅ グラデーション効果
- ✅ ホバーエフェクト
...
### 🚀 デプロイ
- 本番環境: https://commons-webapp.pages.dev/
- 最新デプロイ: https://20966dcf.commons-webapp.pages.dev/
- GitHub: https://github.com/ryufuji/commnos
- コミット: 6e9ce94

### 🎯 変更統計
- バンドルサイズ: 974.52 kB
- 変更行数: tenant-public.ts (76行追加, 58行削除), commons-components.css (187行追加)
- 削除されたファイル参照: `/static/styles.css` (完全削除)
