...
このプロンプトには「既存プロジェクトのファイルを参照してください」という記述がありますが、**Claude Codeの新規セッションでは既存ファイルにアクセスできません**。

### 対処方法

1. **このプロンプトを使用する前に、ユーザーに以下を依頼してください：**
   - 既存プロジェクトの全ファイルを Claude Code にアップロード
   - または、GitHub リポジトリ URL を提供してもらい、git clone で取得

2. **必要なファイル（合計約80ファイル）：**
   - `migrations/*.sql` - 36個のマイグレーションファイル
   - `src/routes/*.ts` - 36個のAPIルートファイル
   - `public/static/*` - 6個のフロントエンドファイル
   - `src/index.tsx` - メインアプリケーション（2700行）
   - その他の設定ファイル
...
3. **GitHub からクローンする場合：**
   ...
   mkdir -p webapp
   # 既存プロジェクトのファイルを参照: existing-project/
   # 新規プロジェクトの構築先: webapp/
   ```

4. **ファイルがアクセスできない場合の対応：**
   - 基本的な実装のみを行い、詳細は後でユーザーに追加実装を依頼する
   - または、このプロンプトの範囲内で実装可能な部分のみを完成させる
