# Commons - Phase 7 開発中！チケット・物販システム
...
- **ポイントシステム完全実装** 🎉
  ...
  - ✅ 管理者による柔軟なルール設定
  - ✅ 報酬管理・交換申請システム
  - ✅ 自動付与（デイリーログイン、記事閲覧、コメント、プロフィール完成）

🚧 **Phase 7 開発中（チケット・物販システム）:**
- **特定商取引法対応** - 必須事業者情報の登録・管理
  - ✅ データベース設計完了（shop_legal_info テーブル）
  - ⏳ 事業者情報登録API
  - ⏳ 管理画面：事業者情報入力フォーム
  - ⏳ 特定商取引法表記ページ自動生成
- **商品・チケット管理**
  - ✅ データベース設計完了（shop_products, shop_categories）
  - ⏳ 管理画面：商品登録・編集機能
  - ⏳ 在庫管理機能
  - ⏳ 販売期間設定
  - ⏳ カテゴリ管理
- **注文・決済システム**
  - ✅ データベース設計完了（shop_orders, shop_order_items）
  - ⏳ 会員向け：商品購入ページ
  - ⏳ Stripe決済連携（商品決済）
  - ⏳ 注文管理画面
  - ⏳ 配送状況管理
...
**Phase 7 の特徴:**
- **法令遵守**: 特定商取引法の必須項目を完全に網羅
- **情報未入力時は販売不可**: 法的要件を満たすまで販売機能をロック
- **柔軟な商品タイプ**: 物理商品/チケット/デジタルコンテンツに対応
- **Stripe決済統合**: 既存のサブスクリプション決済に加えて単発商品決済を追加
...
Commonsは、**2つのレイヤー**で構成されるマルチテナントプラットフォームです：

#### フェーズ1: プラットフォームレイヤー（B2B）
**VALUE ARCHITECTS（サービス運営者） ⇄ コミュニティ運営者（オーナー）**

- **提供するもの**: プラットフォーム利用権、インフラリソース（ストレージ、メンバー数上限）
- **課金対象**: コミュニティ運営者
- **料金体系**:
  - **フリー**: ¥0/月（50人まで、1GB）
  - **スターター**: ¥3,000/月（100人まで、5GB）
  - **成長**: ¥10,000/月（500人まで、50GB）
  - **エンタープライズ**: ¥30,000/月（無制限、500GB）
- **手数料**: メンバー収益の15-20%
...
- **課金対象**: 一般会員
- **料金**: コミュニティ運営者が自由に設定（例: ¥980 - ¥2,980/月）
- **収益**: 手数料控除後の80-85%を受け取る

詳細は [`docs/STORAGE_ARCHITECTURE.md`](/docs/STORAGE_ARCHITECTURE.md) を参照してください。

### データモデル

**主要テーブル（13テーブル）:**
1. **users** - ユーザー情報（ID、メール、パスワード、ニックネーム、アバター、bio、ステータス）
2. **tenants** - テナント情報（ID、サブドメイン、名前、オーナー、プラン、ストレージ、Stripe情報、テーマ、platform_plan_id）
3. **platform_plans** - プラットフォームプラン定義（フェーズ1: 料金、メンバー上限、ストレージ上限、手数料率）✨ NEW
4. **tenant_plans** - メンバー向けプラン（フェーズ2: コミュニティ独自のプラン）
5. **tenant_memberships** - テナント-ユーザー関連（役割、会員番号、ステータス、期限、plan_id）
6. **posts** - 投稿（タイトル、コンテンツ、ステータス、公開日時、閲覧数）
7. **comments** - コメント（内容、親コメントID、作成日時）✨ Phase 2
8. **tenant_customization** - テナントカスタマイズ設定（テーマプリセット）
9. **tenant_features** - テナント機能設定
10. **tenant_earnings** - テナント収益管理（残高、累計収益、手数料率）
11. **earnings_transactions** - 収益トランザクション履歴
12. **platform_admins** - プラットフォーム管理者（VALUE ARCHITECTS専用）
13. **payment_history** - 決済履歴
